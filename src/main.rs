...
println!("{}@{}", user.unwrap().name(), option_to_string(sys.host_name().as_deref()));
 // Display system information:
println!("OS:           {}", option_to_string(sys.long_os_version().as_deref()));
println!("Kernel:       {}", option_to_string(sys.os_version().as_deref()));
println!("Uptime:       {}", convert_second_to_humantime(sys.uptime()));
println!("Processes:    {}", sys.processes().len());
println!("Users:        {}", sys.users().len());
println!("Boot time:    {}", convert_second_to_humantime(sys.boot_time()));
sys.refresh_cpu(); // Refreshing CPU information.
for cpu in sys.cpus() {
    println!("{}% ", cpu.brand());
    break;
}
println!("Memory: {} Mib / {} Mib", sys.used_memory()/1024/1024, sys.total_memory()/1024/1024);
